...
import (
	...
	"github.com/sentiae/infrastructure-intelligence-service/migrations"
)
...
// the old GORM AutoMigrate + ApplyRLSObjects boot path (D-178).
// migrate.ErrNoChange is not an error (an already-current DB is a clean no-op).
//
// MUST be called under the OWNER connection (augur_service_owner): the baseline
// alters table RLS state and transfers resolver-function ownership to
// augur_service_system, which the NOBYPASSRLS app role cannot do.
//
